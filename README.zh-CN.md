...
> 当前状态：设计规格与 v1 实施计划阶段。该仓库不是量产级安全认证产品。
...
Starling 同时包含开源硬件、软件和文档，不适合只用一个普通软件协议覆盖全部内容。推荐分层授权：

| 项目部分 | 推荐协议 | 原因 |
|---|---|---|
| 硬件设计文件：CAD、PCB、原理图、Gerber、机械图纸、BOM | [CERN-OHL-S-2.0](https://cern-ohl.web.cern.ch/) | 面向硬件设计，强互惠，鼓励安全相关改型和设计改进保持开放 |
| 软件：固件、App、云函数、Web 控制台、Python 分析器、脚本 | [Apache-2.0](https://www.apache.org/licenses/LICENSE-2.0) | 宽松、软件生态熟悉，并包含明确专利授权条款 |
| 文档：规格、计划、手册、测试流程、报告 | [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/) | 保留署名要求，并要求衍生文档以相同方式共享 |

如果未来更重视商业扩散而不是硬件改进回流，可以把硬件协议从 CERN-OHL-S-2.0 调整为 CERN-OHL-W-2.0 或 CERN-OHL-P-2.0。若目标是兼容开源硬件实践，应避免使用 Creative Commons 的“非商业”变体。

公开发布前：

1. 添加根目录协议文件，例如 `LICENSES/CERN-OHL-S-2.0.txt`、`LICENSES/Apache-2.0.txt` 和 `LICENSES/CC-BY-SA-4.0.txt`。
2. 添加 `LICENSES.md`，说明每个目录适用的协议。
3. 在可行文件中添加 SPDX 头或短版权声明。
4. 除非你明确想授权品牌，否则保留 Starling 名称和 logo 的商标权。
5. 发布前清除密钥、本地 token、私有路径和所有凭据。
