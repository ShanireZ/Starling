...
> **采购执行版(2026-07-02 零烙铁审核)**:[`res/prototype-v1-采购清单.xlsx`](../res/prototype-v1-采购清单.xlsx) —— 含全部辅材(线缆/胶枪/端子/轴材等 37 项)、数量与备件策略、淘宝搜索词、免焊要点;台架页必买合计约 ¥615(全部 ¥826)。关键增补:模块一律选**已焊排针**发货、空速计套装标配 **JST-GH 座须配转杜邦线**、DS3231 须 **LIR2032 充电版**(CR2032 要拆电阻=焊接活)、新增 **ESP32 38Pin 端子扩展板**、机构联动改 **M2 球头拉杆+易拉夹**(免弯钢丝)。**PCB 结论**:台架阶段不打板(全免焊可跑通);装车阶段做**模块底板**(嘉立创贴片+插件焊,排母插模块),不做芯片级全集成(详见 xlsx「说明与PCB方案」页)。
