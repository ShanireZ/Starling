...
- **刹车 / 减速**：翼片立起 0-70° → 产生大面积气动阻力作为辅助减速器
...
- 不需要倾斜驱动即可获得"基础下压力"
- 通过驱动到 -5° 可"取消"camber 效果，趋近零阻力（v2 DRAG_REDUCE 状态用）
...
- **v1**：SLA 结构树脂（Formlabs Tough 2000 或同级）+ 内嵌 ∅3mm 碳棒纵梁，便于迭代翼型
- **v2**：CNC 6061 铝芯 + 碳纤湿布 layup，外形不变内部加强
- **v3**：碳纤预浸料 prepreg autoclave 工艺（如有量产需要）
...
typedef struct __attribute__((packed)) {
    // 时间 (12B)
    uint64_t time_us;
    uint32_t gps_time_unix;
    // GPS (24B)
    int32_t  gps_lat_e7, gps_lon_e7;
    int16_t  gps_alt_dm;
    uint16_t gps_speed_kmh_cx100;
    uint8_t  gps_fix_type, gps_hdop, gps_sats;
    uint8_t  pad1[5];
    // IMU 原始 (24B)
    int16_t  ax, ay, az, gx, gy, gz;
    uint8_t  pad2[12];
    // 姿态融合 (8B)
    int16_t  roll_cdeg, pitch_cdeg, yaw_cdeg;
    int16_t  linear_accel_long_cms2;
    // 控制相关 (24B)
    uint16_t wheel_speed_kmh_cx100;
    uint8_t  brake_lever_active, state_current;
    int16_t  target_angle_l_cdeg, target_angle_r_cdeg;
    int16_t  measured_angle_l_cdeg, measured_angle_r_cdeg;
    uint16_t servo_current_l_ma, servo_current_r_ma;
    uint8_t  pad3[8];
    // 健康 (8B)
    uint8_t  fault_flag, health_bits;
    uint16_t supply_voltage_mv, supercap_voltage_mv;
    uint16_t cpu_loop_us;
    // 杂项 (28B)
    uint16_t lookup_table_version;
    uint8_t  pad4[24];
    // CRC (2B)
    uint16_t crc16;
} starlog_frame_t;  // 总长 128 字节
```
...
| **v2** | 双车适配 + 外层 PID + iOS App + Savox 升级 + 碳纤翼片 + 应变监测 | **3-4 月** | +6-8K RMB | 5000 km 耐久达成 |
